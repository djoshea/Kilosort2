                 ...
                 int nrhs, mxArray const *prhs[])
{
  ...
  // @djoshea also copy d_st1 and d_id1 to enable reproducible 
  const mwSize dstout[] {counter[0], 1};
  mxGPUArray *stout, *idout;
  
  if (nlhs > 2) {
      stout = mxGPUCreateGPUArray(2, dstout, mxINT32_CLASS, mxREAL, MX_GPU_DO_NOT_INITIALIZE);
      int* d_stout	= (int *)(mxGPUGetData(stout));
      if (counter[0]>0)
          cudaMemcpy(d_stout, d_st1, counter[0] * sizeof(int), cudaMemcpyDeviceToDevice);
  }

  if (nlhs > 3) {
      idout = mxGPUCreateGPUArray(2, dstout, mxINT32_CLASS, mxREAL, MX_GPU_DO_NOT_INITIALIZE);
      int* d_idout = (int *)(mxGPUGetData(idout));
      if (counter[0]>0)
          cudaMemcpy(d_idout, d_id1, counter[0] * sizeof(int), cudaMemcpyDeviceToDevice);
  }
  ...
  if (nlhs > 2)
    plhs[2]   = mxGPUCreateMxArrayOnGPU(stout);
  if (nlhs > 3)
    plhs[3]   = mxGPUCreateMxArrayOnGPU(idout);
  ...
  if (nlhs > 2)
    mxGPUDestroyGPUArray(stout);
  if (nlhs > 3)
    mxGPUDestroyGPUArray(idout);
}